...
export async function createInvoice(prevState: any, formData: FormData) {
    ...
    // Obtener la sucursal activa de las cookies
    const cookieStore = await cookies();
    const activeBranchKey = cookieStore.get('active_branch_id')?.value || 'vivero-del-sol';
    const activeBranchName = cookieStore.get('active_branch_name')?.value || 'Vivero del Sol';
    
    // Por ahora, no guardamos branch_id como UUID hasta que tengamos la tabla de sucursales
    // Solo guardamos el nombre de la sucursal
    const activeBranchId = null;
    
    const { lineDiscounts, appliedPromos } = await applyPromotions({
        supabase,
        items: products,
        branchId: activeBranchKey,
    });
