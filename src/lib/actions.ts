    ...
    try {
        ...
        if (e instanceof z.ZodError) {
             const formattedErrors = Object.entries(e.flatten().fieldErrors)
                .map(([field, messages]) => `${field}: ${messages?.join(', ') || 'Error desconocido'}`)
                .join('; ');
...
export async function addProvider(prevState: any, formData: FormData) {
    ...
    let final_provider_type_code = provider_type_code === 'add_new' ? new_provider_type_code : provider_type_code;
    let final_provider_type_description = provider_type_code === 'add_new' ? new_provider_type_description : null;

    // Si se está creando un nuevo tipo, guardarlo en la tabla provider_types
    if (provider_type_code === 'add_new' && new_provider_type_code && new_provider_type_description) {
        // Verificar si el tipo ya existe en provider_types
        const { data: existingType } = await supabase
            .from('provider_types')
            .select('code')
            .eq('code', new_provider_type_code)
            .single();
        ...
        if (!existingType) {
            const { error: typeError } = await supabase
                .from('provider_types')
                .insert({
                    code: new_provider_type_code,
                    description: new_provider_type_description
                });
            
            if (typeError) {
                return { message: `Error creando el tipo de proveedor: ${typeError.message}` };
            }
        }
        
        final_provider_type_code = new_provider_type_code;
        final_provider_type_description = new_provider_type_description;
    } else if (final_provider_type_code && provider_type_code !== 'add_new') {
        // Obtener la descripción del tipo existente desde provider_types
        const { data: existingType } = await supabase
            .from('provider_types')
            .select('description')
            .eq('code', final_provider_type_code)
            .single();
        
        if (existingType) {
            final_provider_type_description = existingType.description;
        } else {
            // Fallback: buscar en la tabla providers (para compatibilidad)
            const { data: fallbackType } = await supabase
                ...
                .single();
            if (fallbackType) {
                final_provider_type_description = fallbackType.provider_type_description;
            }
        }
    }
...
const basePromotionSchema = z.object({
  name: z.string().min(3, "El nombre debe tener al menos 3 caracteres."),
  is_active: z.string().transform((val) => val === 'true'),
  ...
  can_be_combined: z.string().transform((val) => val === 'true'),
...
// Esquema que maneja correctamente strings vacíos
const baseClientSchema = z.object({
  ...
  razon_social: z.string().transform(val => val === '' ? undefined : val).optional(),
  nombre_fantasia: z.string().transform(val => val === '' ? undefined : val).optional(),
  iva_condition: z.string().transform(val => val === '' ? undefined : val).optional(),
  document_type: z.string().transform(val => val === '' ? undefined : val).optional(),
  document_number: z.string().transform(val => val === '' ? undefined : val).optional(),
  price_list: z.string().transform(val => val === '' ? undefined : val).optional(),
  province: z.string().transform(val => val === '' ? undefined : val).optional(),
  address: z.string().transform(val => val === '' ? undefined : val).optional(),
  city: z.string().transform(val => val === '' ? undefined : val).optional(),
  postal_code: z.string().transform(val => val === '' ? undefined : val).optional(),
  phone: z.string().transform(val => val === '' ? undefined : val).optional(),
  mobile_phone: z.string().transform(val => val === '' ? undefined : val).optional(),
  birth_date: z.string().transform(val => val === '' ? undefined : val).optional(),
  
  // Campos especiales que requieren validación específica
  email: z.string().transform(val => val === '' ? undefined : val).optional().refine(
    (val) => !val || z.string().email().safeParse(val).success,
    { message: 'El email no es válido.' }
  ),
  default_invoice_type: z.string().transform(val => val === '' ? undefined : val).optional().refine(
    (val) => !val || ['A', 'B', 'C'].includes(val),
...
const updateClientSchema = z.object({
    // Campo requerido para identificar el cliente
    id: z.coerce.number(),
    ...
    // Filtrar campos undefined, manteniendo siempre id, name y last_name
    const result: any = {
        id: data.id,
        name: data.name,
    ...
    Object.entries(data).forEach(([key, value]) => {
        if (key !== 'id' && key !== 'name' && key !== 'last_name' && value !== undefined) {
            result[key] = value;
        }
    });
