...
import { createSizeAndAssignProducts } from '@/lib/actions';
...
export function CreateSizeForm({ allProducts, onActionCompleted }: { allProducts: Product[], onActionCompleted?: () => void }) {
    const [state, formAction] = useActionState(createSizeAndAssignProducts, { message: '' });
        ...
        <Dialog open={isDialogOpen} onOpenChange={onDialogChange}>
            ...
            <DialogContent className="sm:max-w-3xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                       Define el nombre del nuevo tamaño y selecciona los productos que lo tendrán.
                    </DialogDescription>
                </DialogHeader>
                <form action={formAction} ref={formRef} className="grid gap-4 py-4">
                     <input type="hidden" name="productIds" value={selectedProductIds.join(',')} />
                    <div className="space-y-2">
                        <Label htmlFor="newSizeName">Nombre del Nuevo Tamaño</Label>
                        <Input id="newSizeName" name="newSizeName" placeholder="Ej: 100cm"/>
                    ...
                    <div>
                        ...
                        <p className='text-sm text-muted-foreground'>Selecciona los productos para asignarles este nuevo tamaño.</p>
                            ...
                            <TableBody>
                                {filteredProducts.map(product => (
                                    <TableRow key={product.id}>
                                        ...
                                        <TableCell>
                                            {product.tamaño ? (
                                                 <Badge variant="secondary">{product.tamaño}</Badge>
                                            ) : (
