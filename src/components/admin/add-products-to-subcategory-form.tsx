...
function SubmitButton() {
    ...
    return (
        <Button type="submit" disabled={pending}>
            {pending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Añadiendo...</> : 'Añadir a la Subcategoría'}
        </Button>
    )
}

export function AddProductsToSubcategoryForm({ subcategoryName, allProducts, onActionCompleted }: { subcategoryName: string, allProducts: Product[], onActionCompleted: () => void }) {
    ...
    const productsNotInThisSubcategory = useMemo(() => {
        return allProducts.filter(p => p.subcategory !== subcategoryName);
    }, [allProducts, subcategoryName]);

    const filteredProducts = useMemo(() => {
        if (!searchTerm) return productsNotInThisSubcategory;
        return productsNotInThisSubcategory.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [productsNotInThisSubcategory, searchTerm]);
    ...
    return (
        <Dialog open={isDialogOpen} onOpenChange={onDialogChange}>
            ...
            <DialogContent className="sm:max-w-3xl">
                <DialogHeader>
                    <DialogTitle>Añadir Productos a la Subcategoría: <span className='font-bold'>{subcategoryName}</span></DialogTitle>
                    <DialogDescription>
                       Selecciona los productos que deseas mover a esta subcategoría.
                    </DialogDescription>
                </DialogHeader>
                <form action={formAction} ref={formRef} className="grid gap-4 py-4">
                    ...
                    <div>
                        ...
                        <p className='text-sm text-muted-foreground'>Selecciona los productos para mover a esta subcategoría.</p>
