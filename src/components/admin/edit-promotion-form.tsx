...
export function EditPromotionForm({ promotion, setOpen }: { promotion: Promotion, setOpen: (open: boolean) => void }) {
  ...
  // Reinicializar estado cuando cambie la promoción
  useEffect(() => {
    setName(promotion.name);
    setIsActive(promotion.is_active);
    setCanBeCombined(promotion.can_be_combined);
    setUsageLimitType(promotion.usage_limit_type);
    setCustomTag(promotion.custom_tag || '');
  }, [promotion]);
