...
import { AssignProvidersToTypeForm } from './assign-providers-to-type-form';
...
export function ProviderTypeManager({ allProviders, allProviderTypes, onActionCompleted }: { allProviders: Provider[], allProviderTypes: ProviderType[], onActionCompleted: () => void }) {
    ...
    const providerCounts = useMemo(() => {
        ...
        for (const type of allProviderTypes) {
            // Filtrar placeholders al contar proveedores reales
            counts[type.code] = allProviders.filter(p => 
                p.provider_type_code === type.code && 
                !p.name.startsWith('[TIPO]') && 
                !p.name.startsWith('__TYPE_PLACEHOLDER_')
            ).length;
        }
        return counts;
        ...
        <Card>
             <CardHeader>
                <div className="flex justify-between items-start">
                    ...
                    <AssignProvidersToTypeForm 
                        allProviders={allProviders} 
                        allProviderTypes={allProviderTypes} 
