...
export function InvoiceView({ invoice, client, company, cashAccounts }: { invoice: Invoice, client: Client | null, company: CompanyData | null, cashAccounts: CashAccount[] }) {
    ...
    const productLines: InvoiceProductLine[] = Array.isArray(invoice.products) ? invoice.products as InvoiceProductLine[] : [];
    ...
    // Parsear las promociones aplicadas (pueden venir como string JSON)
    let promotionsApplied: AppliedPromo[] = [];
    if (Array.isArray(invoice.promotions_applied)) {
        promotionsApplied = invoice.promotions_applied as AppliedPromo[];
    } else if (typeof invoice.promotions_applied === 'string' && invoice.promotions_applied) {
        try {
            promotionsApplied = JSON.parse(invoice.promotions_applied) as AppliedPromo[];
        } catch (error) {
            console.error('Error parsing promotions_applied:', error);
            promotionsApplied = [];
        }
    }
                     ...
                     <div className="flex justify-end pt-6">
                        <div className="w-full max-w-sm text-right space-y-2 text-gray-700">
                            ...
                            {promotionsApplied.length > 0 && (
                                <>
                                    {promotionsApplied.map((promo, index) => (
                            ...
                            {/* Solo mostrar descuento general si existe y es mayor a 0 */}
                            {generalDiscount > 0 && (
                                <div className="flex justify-between text-destructive">
                                    <span>Descuentos</span>
                                    <span className="font-mono">- {formatPrice(generalDiscount, 'ARS')}</span>
                                </div>
                            )}
